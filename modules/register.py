...
def register():
    while True:
        ...
        if c==1:
            ...
            table.columns.header=["ITEM_CODE",'   NAME        ','BRAND','EARNING PER ITEM','ITEMS SOLD','TOTAL PROFIT']
            for row in data:
            ...
            data=mysql_csr.fetchall()
            table = BeautifulTable()
            table.columns.header=["ITEM_CODE",'BEST SELLER','BRAND','EARNING','ITEMS SOLD','PROFIT']
            for row in data:
                table.rows.append(row)
            print(table)
            ...
            mysql_csr.execute(profitquery+f"AND productinfo.ITEM_CODE ='{code}'")
            data=mysql_csr.fetchall()
            table = BeautifulTable()
            table.columns.header=["ITEM_CODE",'   NAME        ','BRAND','EARNING PER ITEM','PROFIT']
            for row in data:
                table.rows.append(row)
            print(table)
            ...
            table.columns.header=["ITEM_CODE",'NAME','BRAND',"SUPPLIER NAMES",'TOTAL_PAID_INVESTMENT']
            for row in data:
                table.rows.append(row)
            print(table)
