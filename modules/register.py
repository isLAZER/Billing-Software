...
def register():
    while True:
        ...
        if c==1:
            ...
            print("Supplier logs:-")
            mysql_csr.execute("SELECT ITEM_CODE,ITEM_NAME,BRAND,AMT_PAID,SUPP_NAME,AMT_PAID*STOCK as TOTAL_PAID FROM register,supplier where SUPP_ID=SUPPLIER_ID")
            data=mysql_csr.fetchall()
            table = BeautifulTable()
            table.columns.header=["ITEM_CODE",'NAME','BRAND','STOCK',"SUPPLIER NAMES",'TOTAL_PAID']
            for row in data:
                table.rows.append(row)
            print(table)
