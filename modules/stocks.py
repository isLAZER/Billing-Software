...
def shop():
    while True:
        ...
        # deleting an entry
            ...
            code = input(
                "Enter the code of the item you want to remove from database: ")
            try:
                mysql_csr.execute(
    ...
    while True:
        ...
        if x == 1:
            ...
            table.columns.header = [
                "CODE", 'NAME', 'BRAND', 'ITEM SOLD', 'STOCK LEFT', ' STATUS']
            if count == 0:
        ...
        # updating stocks
            ...
            while True:
                c = int(input(
                    '(1) Adding stock amount for an item\n(2) Adding stock based on category\n..> '))
                if c == 1:
                    code = input(
                        'Enter the Code of the Item to update stocks of: ')
                    getstockinfo(code)
                    print()
                    quantity = int(input("Enter quantity to be added: "))
                    try:
                        mysql_csr.execute(
                            f"UPDATE productinfo,stocks SET STOCK = STOCK+{quantity} where stocks.ITEM_CODE=productinfo.ITEM_CODE AND productinfo.ITEM_CODE='{code}' ")
                        ms.commit()
                    ...
                    code = input(
                        'Enter the Category of items to update stocks of: ')
                    ...
                    print()
                    quantity = int(
                        input("Enter quantity to be added to all items above: "))
                    try:
                        mysql_csr.execute(
                            f"UPDATE productinfo,stocks SET STOCK = STOCK+{quantity} where stocks.ITEM_CODE=productinfo.ITEM_CODE AND productinfo.CATEGORY= '{code}' ")
                        ms.commit()
                        print("Amount updated!")
                        print()
                        print("Updated Record:-")
